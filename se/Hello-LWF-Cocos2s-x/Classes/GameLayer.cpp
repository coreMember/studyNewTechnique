...
void GameLayer::initEnemy()
{
    ...
    //敵の表示(LWF)
    const char *path = StringUtils::format("Enemy%d/Enemy%d.lwf", _level, _level).c_str();
    _enemy = LWFNode::create(path);
    _enemy->lwf->rootMovie->SetX(100);
    _enemy->setPosition(Point(WINSIZE.width/2, WINSIZE.height/2));
    ...
    core->setPosition(Point(_enemy->getPositionX(), _enemy->getPositionY()));
    ...
    core->timer->setPosition(Point(_enemy->getPositionX(), _enemy->getPositionY()));
...
void GameLayer::initMembers()
{
    ...
    {
        "CardBlue/CardBlue.lwf",
        "CardRed/CardRed.lwf",
        "CardGreen/CardGreen.lwf",
        "CardYellow/CardYellow.lwf",
        "CardPurple/CardPurple.lwf"
    };
    ...
    for (int i = 0; i < fileNames.size(); i++)
    {
        ...
        //メンバーの表示
        auto member = LWFNode::create(fileNames[i].c_str());
...
//    }
    ...
    if(afterHp <= 0){
        member->lwf->playing = false;
        member->runAction(MoveBy::create(2.0, Point(0, -100)));
    }
